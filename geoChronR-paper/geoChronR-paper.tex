...
Some primary publications of new records incorporate time-uncertain analysis into their studies \citep[e.g.,][]{Boldt2015, falster2018millennial, khider2014}, but this remains rare.
...
GeoChronR interfaces with each of these algorithms through their R packages \citep{baconPackage, oxcAAR} (cite other R Packages), streamlining input and output. \%data input and the extraction of the age ensembles from the MCMC results for further analysis.
...
In particular, the \texttt{outlierProbs} parameter is useful in giving less weigth to chronological tie points that may be considered outliers either because they create a reversal in the stratigraphic sequence or they were flagged during analysis (e.g.~contamination).
...
\def\labelenumi{\arabic{enumi}.}
\item
  Adjusting the p-values by the effective sample size to reflect the reduction in degrees of freedom due to autocorrelation following \citet{bretherton1999};
\item
  \{isopersistence\} (Julien your input here would be great)
\item
  \{ebisuzaki\}
\end{enumerate}
...
Isopersistence\ldots{}. Ebisuzaki\ldots{}. , which is why we consider the most broadly applicable, and is the default choice in GeoChronR.
...
The two timeseries exhibit consistently negative correlations, although 19\% of the ensemble members are positive.
In this example, the isopersistent approach finds the most significant correlations, with 19.8\% signficant ensemble members, and 0\% remain significant after adjusting for false discovery rate.
As expected, the Ebisuzaki approach is more conservative, with only 1.7\% significant members, none of which remain signficant with FDR.
...
As mentioned above, there are many reasons to believe that there are teleconnections that link Greenland temperatures to the dynamics monsoon circulation in Asia, especially during abrupt climate changes during glacial periods \citep[e.g.][ ]{liu2013links, duan2016onset, zhang2019asian}.
...
The impact of age uncertainty on the correlation of this record is apparent; even when correlated against itself, only \texttt{1}\% of the ensembles have r-values greater than 0.9, and the median correlation is \texttt{0.73}.
