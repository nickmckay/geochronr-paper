...
```{r age-model,out.width = "12cm",echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
print(chronPlot)
```
...
```{r timeseries,out.width = "12cm",echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
...
```{r cor-hist,out.width = "8.3cm",echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
print(corPlot)
```
...
```
```{r hulu-cor-hist,out.width = "8.3cm",echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
print(corPlotHulu)
...
As in correlation and regression, aligning the data onto a common timescale is requiered for ensemble PCA.
All but two of these datasets are annually resolved, and the other two have 5-year resolution, so it's reasonable to bin these data into 5 year bins.
...
Because the data analyzed here have variable units, and we're not interested in the magnitude of the variance only the relative variability between the datasets,  we choose to use a correlation, rather than covariance, matrix.
...
The scree plot illustrates how the variance explained by each component declines as the component number increases for both the data, and the null hypothesis. 
Due to age uncertainty, the variance explained is a distribution of results, which we compare to the 95th quantile of the null hypothesis ensemble.
Figure \@ref(fig:scree) indicates that the first two components are clearly distinguished from the null hypothesis.
The third component is borderline, with the variance explained by the median of the ensemble near the null hypothesis benchmark.
Therefore, we will focus our investigation on the first two components of the results.
...
There is a much larger impact of age uncertainty on the loadings in PC2 than in PC1, which illustrated by the size of the markers on the map, which are inversely related to the standard deviation of the loadings across the ensemble PCA results, such that smaller markers indicate larger uncertainties.
The PC2 timeseries includes more multidecadal variability than PC1 and is more impacted by age uncertainty.
A key feature of the timeseries is a peak in values in the late 20th century, that occurs after the pronounced peak in PC1.
This suggests that unlike the mid-20th century peak in warming apparent in most of the data, this later warming was dominated by southern Greenland, and counterbalanced by a decline in values in the northern Atlantic Arctic.
...
To do so, they select the benthic $\delta^{18}\mathrm{O}$ record of the International Ocean Drilling Project core 846 [@mix1995benthic, @Shackleton95], covering the past 4.7 million years. 
...
I <- lipdR::readLipd("~/Downloads/ODP846.Lawrence.2006.lpd")
...
As mentioned in \@ref(sec:spec_theory) a standard assumption of spectral analysis is that data are evenly spaced in time. 
...
From this point there are two ways to proceed: 1) use methods that explictly deal with unevenly-spaced data, or 2) interpolate to a regular grid and apply standard methods (see \@ref(sec:spec_theory)). 
...
On the other hand, MTM [@thomson82] is an optimal estimator, which is consistent (the more data, the better constrained the amplitude of the peaks). 
...
Conveniently, both the interpolation routine and MTM are available within the  [astrochron @astrochron] package, which is a dependency of GeoChronR.
...
For consistency with REDFIT, we define the null as an AR(1) process fit to the data, but GeoChronR suports two other nulls: a power-law null and one derived with the method of @MannLees96. 
...
```{r MTM ensemble export,out.width = "12cm",echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "MTM spectrum for IODP 846, 1000-member age ensemble"}
print(p.mtm.ens)
```
