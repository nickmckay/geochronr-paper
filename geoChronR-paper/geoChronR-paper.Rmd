...
Fortunately, @bronkramsey2010 made it possible for *k* to be treated as a variable, and the model will estimate the most likely values of *k* given a prior estimate and the data. The downside of this flexibility is that this calculation can greatly increase the convergence time of the model. Oxcal is written in C++, with an interface in R [@oxcAAR]. Oxcal does not typically calculate posterior ensembles for a depth sequence, but can optionally output MCMC posteriors at specified levels in the sequence. GeoChronR uses this feature to extract ensemble members for subsequent analysis.
...
Therefore, in addition to the standard Student's T-test p-value calculation, we also estimate the effect of autocorrelation on significance using three commonly used approaches:

1. Adjusting the p-values by the effective sample size to reflect the reduction in degrees of freedom due to autocorrelation following @bretherton1999;

Julien - help
2. {isopersistent test} AR(1) null 

3. { isospectral test } @Ebisuzaki_JClim97 introduced a phase-scrambling technique that preserves the series' individual spectra. rEDM package ref. 



Each of these approaches has strengths and weaknesses. The effective sample size approach is fast, and provides reasonable results for moderately autocorrelated timeseries, but can be overly conserative for highly autocorrelated data. This also makes the least stable of the methods to changes in bin size. Isopersistence.... Ebisuzaki.... , which is why we consider the most broadly applicable, and is the default choice in GeoChronR.

In addition to the impact of autocorrelation on this analysis, repeating the test over multiple ensembles raises the issue of test multiplicity  [@Ventura2004], or "look elsewhere effect".
...
Correlating the two age-uncertain datasets will characterize the relationship, but lumps together all timescales, and ignores any ancillary information that may support a mechanistic relationship between two timeseries.
However, it does provide insight into how age uncertainty can affect a crude measure of similarity between two paleoseries.
...
As expected, the Ebisuzaki approach is more conservative, with only `r round(sum(corout$cor.df$pEbisuzaki<0.05)/nrow(corout$cor.df)*100,digits = 1)`% significant members, none of which remain signficant with FDR.
Lastly, the effective sample size approach of @bretherton1999 is most conservative, finding no significant correlations.

Despite the variability between approaches, given the age models and correlation parameters used here, it seems unlikely that these two datasets are not significantly correlated.
As mentioned above, there are many reasons to believe that there are teleconnections that link Greenland temperatures to the dynamics monsoon circulation in Asia, especially during abrupt climate changes during glacial periods [@refs].
However, age-uncertain correlation between these two records are not among them.
It is worth noting that evaluating the significancce of age uncertain correlation remains somewhat subjective, as there is no theoretical expectation for what fraction of ensemble correlation results should be expected to pass such a significance test.
...
The message, thus, is that age uncertainties will obscure relationships between records, while in some cases creating the appearance of spurious correlations. 
Generally, it is appropriate to think of the ensemble correlation results produced by GeoChronR as a first-order estimate of the age-uncertain correlation characteristics between timeseries, rather than a binary answer to the question "Are these two datasets significantly correlated?".
