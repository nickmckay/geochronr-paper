...
In this example, the isopersistent approach finds the most significant correlations, with `r sprintf("%.1f",sum(corout$cor.df$pIsopersistent<0.05)/nrow(corout$cor.df)*100)`% signficant ensemble members, and `r round(sum(corout$cor.df$pIsopersistentFDR)/nrow(corout$cor.df)*100 ,digits = 1)`% remain significant after adjusting for false discovery rate.
