...
```{r, Hulu Chronology export,echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
print(chronPlot)
```
...
```{r, Correlation plot, echo = FALSE,warning=FALSE,message = FALSE,fig.cap = "Need to write a caption"}
print(corPlot)+ggtitle(NULL)
```


In this use case, we demonstrate how a user could calculate and visualize and age-uncertain correlation between the Hulu speleothem $\delta^{18}\mathrm{O}$ record and the GISP2 ice core $\delta^{18}\mathrm{O}$ record. This is illustative the broader task of chronology alignment, whereby age-uncertain records are often "pegged" against a better-constrained chronology by aligning features.
(Introduction about why a user might want to do an age uncertain correlation between Hulu and GISP2).
